...
Murray_Data = embryo[,embryo@phenoData@data$batch == "Murray_r17"]
...
my_path <- "~/Lab_Data/Original_Files/Murray_r17/raw_feature_bc_matrix/"
...
colData(sce_walkthrough)$Batch <- "Murray_r17" # This can be the "batch" value (Murray_b02, 300, etc)
...
knee_point
# = 105
# Using a new value at 450, on the top of the mini slope
...
# 0.0     0.0     0.0     0.3     0.5     1.0  523472 

summary(emptydrops.out$FDR <= 0.01)
#  Mode   FALSE    TRUE    NA's
#logical  10117    9720  523472s
...
# top 15 genes here
# "his-24"    "hil-2"     "hil-7"     "nduo-6"    "ctc-2"     "atp-6"    
# "ttr-50"    "ctc-3"     "clec-266"  "rpl-12"    "ant-1.1"   "lbp-1"    
# "pat-10"    "hsp-16.41" "dpy-14" 
...
sce_walkthrough$matching_barcodes <- paste(substring(sce_walkthrough$Barcode, 1, 17), "r17", sep = "")
head(colData(sce_walkthrough))
...
library(Matrix)
#counts
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/matrix.mtx")
system("gzip ~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/matrix.mtx")
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/matrix.mtx")

#barcodes
fil.barcodes <- colnames(sce_filtered)
write.table(fil.barcodes, "~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
system("gzip ~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/barcodes.tsv")
write.table(fil.barcodes, "~/Lab_Data/SoupX/Murray_r17/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
# NO center blob
# Seurat identified 36 clusters. There is good separation between the clusters
...
my_dir_S <- "~/Lab_Data/SoupX/Murray_r17/"
...
plotMarkerMap(scl.sce, s2i("cut-2", gids)) #bad
...
est.cont <- s2i(c("dpy-14", "sqt-3", "pat-10", "clik-1", "cpn-3", "C02E7.6"), gids)
...
# Estimated global contamination fraction of 14.16%
# 20.89 with nlp-77
...
# 1690 genes passed tf-idf cut-off and 931 soup quantile filter.  Taking the top 100.
# Using 1912 independent estimates of rho.
...
saveRDS(scl.sce, "~/Lab_Data/SoupX/Murray_r17/Murray_r17_soupX_object.rds")
saveRDS(soup.out, "~/Lab_Data/SoupX/Murray_r17/Murray_r17_soupX_corrected_matrix.rds")
...
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.2865  2.6956  3.4753  4.0950  4.7157 37.1893 


summary(sce_corrected$subsets_Mito_percent)
# Min.  1st Qu.  Median    Mean 3rd Qu.    Max. 
#0.06086  2.28493  3.16690  3.84184  4.55323 39.87474 
...
sce
saveRDS(sce, "~/Lab_Data/SoupX/Murray_r17/Murray_r17_uncorrected.rds")

sizeFactors(sce_corrected) <- librarySizeFactors(sce_corrected)
sce_corrected <- logNormCounts(sce_corrected)
sce_corrected
saveRDS(sce_corrected, "~/Lab_Data/SoupX/Murray_r17/Murray_r17_SoupX_corrected.rds")
