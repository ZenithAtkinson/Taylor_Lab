...
embryo@phenoData@data[1:600,1:15]
...
W500 = embryo[,embryo@phenoData@data$batch == "Waterston_500_minutes_batch_2"]

# Subsetting for Waterson 500 minutes
#W500 <- subset(embryo, subset = batch == "Waterson_500_minutes")
# This gives us ~17000, which we can split into the different barcodes
table(W500@phenoData@data$batch)
head(W500@phenoData@data)

W500$Sample = substring(W500@phenoData@data$Cell, 24, 24)

table(W500$Sample)
W500_lane1 <- W500[, W500$Sample == "1"] # Selecting lane1
table(W500_lane1$Sample)
...
my_path <- "~/Lab_Data/Original_Files/Waterston_500_min_batch_2_10X_lane_1/raw_feature_bc_matrix/"
...
colData(sce_walkthrough)$Batch <- "Waterson500_batch2" # This can be the "batch" value (Murray_b02, 300, etc)
...
# Used given lower of 400 intead
...
set.seed(100)
emptydrops.out <- emptyDrops(counts(sce_walkthrough), lower = given_lower) # Takes a while depending on size
head(emptydrops.out)
emptydrops.out
summary(emptydrops.out$FDR, exclude = NULL)

# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
# 0.0     0.0     0.0     0.2     0.4     1.0  692014

summary(emptydrops.out$FDR <= 0.01)
#  Mode   FALSE    TRUE    NA's
#logical  6316    9049  692014 
...
# top 15 genes here
# "hil-7"    "pat-10"   "rpl-12"   "T05E11.9" "cut-2"    "rla-1"   
# "dpy-14"   "sqt-3"    "C02E7.6"  "atp-6"    "rpl-7A"   "mlc-3"   
#  "eef-1A.1" "cpn-3"    "F07H5.3"
...
# Extract the barcode
barcodes_cedata = (pData(W500_lane1)$Cell)
...
# For 500's: batch 1 lane 1 = 500.1.1 (see og embryo data)
#  For 500's: batch 2 lane 1 = 500.2.1

sce_walkthrough$matching_barcodes <- paste(substring(sce_walkthrough$Barcode, 1, 17), "500.2.1", sep = "")
head(colData(sce_walkthrough))
...
# AT THIS POINT: We need to take all of their metadata present in head(pData(W500_lane1)) (like cell.type, cell.subtype, plot.cell.type), and 
...
metadata_W500_lane1 <- pData(W500_lane1)
head(metadata_W500_lane1)
...
library(Matrix)
#counts
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/matrix.mtx")
system("gzip ~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/matrix.mtx")
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/matrix.mtx")

#barcodes
fil.barcodes <- colnames(sce_filtered)
write.table(fil.barcodes, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
system("gzip ~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/barcodes.tsv")
write.table(fil.barcodes, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
# Red = NEW data, blue is equal to old/combined data. Some of the new data is background, but most isnt.
# Seurat identified 46 clusters. There is low seperation between them (big center blob)
...
FeaturePlot(sce_filtered.S, features = s2i("ctc-3", gids))
FeaturePlot(sce_filtered.S, features = s2i("atp-6", gids))

# top 15 genes here
# "hil-7"   "cut-2"   "dpy-14"  "sqt-3"   "pat-10"  "rpl-12"  "atp-6"   "ctc-2"   "F41F3.3" "ctc-3"   "rla-1"   "mlc-3"   "dod-6"  
# "rpl-7A"  "C02E7.6"
...
my_dir_S <- "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/"
...
# Top 8 genes: hil-7, cut-2, dpy-14, sqt-3,pat-10, rpl-12, atp-6, ctc-2
...
plotMarkerMap(scl.sce, s2i("his-24", gids)) # bad
plotMarkerMap(scl.sce, s2i("hil-7", gids)) # bad
plotMarkerMap(scl.sce, s2i("dpy-14", gids)) #very good
plotMarkerMap(scl.sce, s2i("hil-2", gids)) #bad
...
plotMarkerMap(scl.sce, s2i("clik-1", gids)) #good 
plotMarkerMap(scl.sce, s2i("cpn-3", gids)) #very good
plotMarkerMap(scl.sce, s2i("C02E7.6", gids)) #good
plotMarkerMap(scl.sce, s2i("cut-2", gids)) #very good
plotMarkerMap(scl.sce, s2i("pdi-2", gids)) #okay
plotMarkerMap(scl.sce, s2i("rla-1", gids)) #bad

# From soup profile counts: act-4,  sqt-3
# top 15 genes here
# "hil-7"   "cut-2"   "dpy-14"  "sqt-3"   "pat-10"  "rpl-12"  "atp-6"   "ctc-2"   "F41F3.3" "ctc-3"   "rla-1"   "mlc-3"   "dod-6"  
# "rpl-7A"  "C02E7.6"

# Selected genes: ttr-50, dpy-14, sqt-3, pat-10, cpn-3, cut-2
...
est.cont <- s2i(c("dpy-14", "sqt-3", "pat-10", "cpn-3", "cut-2", "clik-1"), gids)
...
# 7815 genes passed tf-idf cut-off and 4032 soup quantile filter.  Taking the top 100.
# Using 3512 independent estimates of rho.
...
saveRDS(scl.sce, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/Waterson_500_lane1_batch2_soupX_object.rds")
saveRDS(soup.out, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/Waterson_500_lane1_batch2_soupX_corrected_matrix.rds")
...
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.000   1.765   2.674   3.463   3.976  38.202 

summary(sce_corrected$subsets_Mito_percent)
# Min.  1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.000   1.063   2.100   3.151   3.719  41.702 
...
sce
saveRDS(sce, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/Waterson_500_lane1_batch2_sce_uncorrected.rds")

sizeFactors(sce_corrected) <- librarySizeFactors(sce_corrected)
sce_corrected <- logNormCounts(sce_corrected)
sce_corrected
saveRDS(sce_corrected, "~/Lab_Data/SoupX/Waterson_500_lane1_batch2/Waterson_500_lane1_batch2_sce_SoupX_corrected.rds")
