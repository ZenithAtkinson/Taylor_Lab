...
Murray_Data = embryo[,embryo@phenoData@data$batch == "Murray_b01"]
...
my_path <- "~/Lab_Data/Original_Files/Murray_b01/raw_feature_bc_matrix/"
...
colData(sce_walkthrough)$Batch <- "Murray_b01" # This can be the "batch" value (Murray_b01, 300, etc)
...
inflect_point
# = 102
# Using a new value at 400, just above the mini plateu
...
# 0.0     0.0    0.1     0.3     0.6     1.0  589800 

summary(emptydrops.out$FDR <= 0.01)
#  Mode   FALSE    TRUE    NA's
#logical  18018   10243  589800 
...
# top 15 genes here
# "hil-7"   "his-24"  "atp-6"   "ctc-2"   "ctc-3"   "nduo-6"  "cav-1"  #
# "ctc-1"   "rpl-12"  "pat-10"  "hil-2"   "crt-1"   "ant-1.1" "rbm-3.2"
# "rrn-3.1"
...
sce_walkthrough$matching_barcodes <- paste(substring(sce_walkthrough$Barcode, 1, 17), "b01", sep = "")
head(colData(sce_walkthrough))
...
library(Matrix)
#counts
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/matrix.mtx")
system("gzip ~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/matrix.mtx")
writeMM(sce_filtered.filt.counts, "~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/matrix.mtx")

#barcodes
fil.barcodes <- colnames(sce_filtered)
write.table(fil.barcodes, "~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
system("gzip ~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/barcodes.tsv")
write.table(fil.barcodes, "~/Lab_Data/SoupX/Murray_b01/filtered_feature_bc_matrix/barcodes.tsv", 
            sep = "\t", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
# Red = NEW data, blue is equal to old/combined data
# No large center blob, new data is in new areas

# Seurat identified 50 clusters. There is good separation between the clusters
...
FeaturePlot(sce_filtered.S, features = s2i("sbt-1", gids))
FeaturePlot(sce_filtered.S, features = s2i("let-858", gids))
FeaturePlot(sce_filtered.S, features = s2i("bnc-1", gids))
FeaturePlot(sce_filtered.S, features = s2i("unc-4", gids))
FeaturePlot(sce_filtered.S, features = s2i("flp-1", gids))
FeaturePlot(sce_filtered.S, features = s2i("unc-122", gids))
...
my_dir_S <- "~/Lab_Data/SoupX/Murray_b01/"
...
plotMarkerMap(scl.sce, s2i("crt-1", gids)) #bad
plotMarkerMap(scl.sce, s2i("ant-1.1", gids)) #bad
plotMarkerMap(scl.sce, s2i("rbm-3.2", gids)) #bad
plotMarkerMap(scl.sce, s2i("rrn-3.1", gids)) #bad
...
est.cont <- s2i(c("dpy-14", "sqt-3", "pat-10", "cpn-3", "clik-1", "cut-2"), gids)
...
# Estimated global contamination fraction of 13.10
...
# 5822 genes passed tf-idf cut-off and 2692 soup quantile filter.  Taking the top 100.
# Using 3457 independent estimates of rho.
...
saveRDS(scl.sce, "~/Lab_Data/SoupX/Murray_b01/Murray_b01_soupX_object.rds")
saveRDS(soup.out, "~/Lab_Data/SoupX/Murray_b01/Murray_b01_soupX_corrected_matrix.rds")
...
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.1761  2.8632  3.7248  4.1669  4.9383 29.3642 


summary(sce_corrected$subsets_Mito_percent)
# Min.  1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.000   2.501   3.469   3.946   4.801  31.879  
...
sce
saveRDS(sce, "~/Lab_Data/SoupX/Murray_b01/Murray_b01_uncorrected.rds")

sizeFactors(sce_corrected) <- librarySizeFactors(sce_corrected)
sce_corrected <- logNormCounts(sce_corrected)
sce_corrected
saveRDS(sce_corrected, "~/Lab_Data/SoupX/Murray_b01/Murray_b01_SoupX_corrected.rds")
